...
class Matrix2D( breve.Matrix ):
	...
	def addValues( self, otherMatrix, scale ):
	...
	def archive( self ):
		...
		return breve.Matrix.archive()


	def computeDiffusionMatrix( self, chemicalMatrix, scale ):
	...
	def computePeriodicDiffusionMatrix( self, chemicalMatrix, scale ):
	...
	def copyToBlueChannel( self, theImage, scale ):
	...
	def copyToGreenChannel( self, theImage, scale ):
	...
	def copyToRedChannel( self, theImage, scale ):
	...
	def dearchive( self ):
		...
		self.setSize( len( self.archiveList ), len( self.archiveList[ 0 ] ) )
		
		return breve.Matrix.dearchive()
	...
	def subtractValues( self, otherMatrix, scale ):
...
class Matrix3D( breve.Matrix ):
	...
	def copySliceToAlphaChannel( self, thePlane, theImage, scale ):
	...
	def copySliceToBlueChannel( self, thePlane, theImage, scale ):
	...
	def copySliceToRedChannel( self, thePlane, theImage, scale ):
	...
	def copySlicedToGreenChannel( self, thePlane, theImage, scale ):
	...
	def copyToAlphaChannel( self, grid, scale ):
	...
	def copyToBlueChannel( self, grid, scale ):
	...
	def copyToGreenChannel( self, grid, scale ):
	...
	def copyToRedChannel( self, grid, scale ):
	...
	def dearchive( self ):
		...
		self.setSize( len( self.archiveList ), len( self.archiveList[ 0 ] ), len( self.archiveList[ 0 ][ 0 ] ) )
...
class MatrixImage( breve.Image ):
	...
	def initWith( self, theMatrix, theScale ):
		'''Initializes the MatrixImage with sizes taken from theMatrix.'''

		breve.Image.initWith( theMatrix.getXDimension(), theMatrix.getYDimension() )
		self.scale = theScale
		return self
