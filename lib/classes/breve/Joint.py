...
class Joint( breve.Abstract ):
	...
	def init( self ):
		...
		self.normalVector
		self.relativeRotation = matrix(  1, 0, 0, 0, 1, 0, 0, 0, 1 )
	...
	def setJointVelocity( self, newVelocity ):
		...
		if self.jointPointer:
			breve.breveInternalFunctionFinder.jointSetVelocity( self, self.jointPointer, breve.vector( newVelocity, newVelocity, 0 ) )
...
class PrismaticJoint( breve.Joint ):
	...
	def link( self, parentLink, childLink, theNormal, parentPoint, childPoint, currentRotation ):
		...
		if ( parentLink and ( not ppointer ) ):
			raise Exception( '''attempting to link uninitialized Link object .''' % (  parentLink ) )


		if ( childLink and ( not cpointer ) ):
			raise Exception( '''attempting to link uninitialized Link object .''' % (  childLink ) )
	...
	def setJointForce( self, forceValue ):
		...
		breve.breveInternalFunctionFinder.jointApplyForce( self, self.jointPointer, breve.vector( forceValue, forceValue, 0 ) )

	def setJointLimits( self, min1, max1 ):
		'''Sets the minimum and maximum allowed positions for this  joint. '''

		breve.breveInternalFunctionFinder.jointSetLimits( self, self.jointPointer, breve.vector( min1, min1, 0 ), breve.vector( max1, max1, 0 ) )
...
class RevoluteJoint( breve.Joint ):
	...
	def setJointTorque( self, torqueValue ):
		...
		breve.breveInternalFunctionFinder.jointApplyForce( self, self.jointPointer, breve.vector( torqueValue, torqueValue, 0 ) )
...
class FixedJoint( breve.Joint ):
	...
	def dearchive( self ):
		...
		print '''dearchiving for  and ''' % (  self.parent, self.child )
		if ( self.parent or self.child ):
	...
	def link( self, parentLink, childLink, parentPoint, childPoint, currentRotation ):
		...
		self.normalVector = breve.vector( 1, 1, 0 )
		self.plinkPoint = parentPoint
		self.clinkPoint = childPoint
		self.parent = parentLink
		self.child = childLink
...
class UniversalJoint( breve.Joint ):
	...
	def link( self, parentLink, childLink, normal, parentPoint, childPoint, currentRotation ):
	...
	def setJointVelocity( self, a1velocity, a2velocity ):
		...
		if self.jointPointer:
			breve.breveInternalFunctionFinder.jointSetVelocity( self, self.jointPointer, breve.vector( a1velocity, a1velocity, 0 ) )
...
class BallJoint( breve.Joint ):
	...
	def link( self, parentLink, childLink, parentPoint, childPoint, normal, currentRotation ):
	...
	def setJointLimits( self, min1, max1, min2, max2, min3, max3 ):
		...
		if self.jointPointer:
			breve.breveInternalFunctionFinder.jointSetLimits( self, self.jointPointer, breve.vector( min1, min1, min3 ), breve.vector( max1, max1, max3 ) )
