...
class Real( breve.Object ):
	...
	def addDottedLine( self, otherObject, theColor ):
	...
	def addLine( self, otherObject, theColor, theStyle ):
	...
	def addMenu( self, menuName, theMethod ):
		...
		newMenu = breve.MenuItem().createMenu( menuName, self, theMethod )
	...
	def addMenuSeparator( self ):
		...
		newMenu = breve.MenuItem().createMenu( '''''', self, '''''' )
		self.addDependency( newMenu )
		self.menus.append( newMenu )
		return newMenu
	...
	def setCollisionShape( self, theShape ):
		...
		if ( not theShape.getPointer() ):
			raise Exception( '''attempt to register Mobile object with uninitialized shape ()''' % (  theShape ) )
	...
	def setRotationAboutAxis( self, thisAxis, amount ):
