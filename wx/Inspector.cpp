...
bool Inspector::Create( wxWindow* parent, wxWindowID id, const wxString& caption, const wxPoint& pos, const wxSize& size, long style )
{
	...
	if (config != NULL) {
		...
		config->Read( "InspectorHidden", &shouldHide );
	}
...
bool Inspector::Show( bool inShow ) {
	wxConfigBase * config = wxConfigBase::Get();

	if ( config != NULL ) 
		config->Write( "InspectorHidden", 0 );

	return wxFrame::Show( inShow );
}
...
void Inspector::OnClose(wxCloseEvent& event) {
	...
	if ( config != NULL ) 
		config->Write( "InspectorHidden", 1 );
...
void Inspector::OnMove(wxMoveEvent &event)
{
	...
	if (config != NULL) {
		...
		config->Write( "InspectorX", position.x );
		config->Write( "InspectorY", position.y );
	}
