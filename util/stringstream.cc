...
#if HAVE_MKSTEMP
	snprintf( nameTemplate, sizeof( nameTemplate ),
	          "/tmp/breve_temp.XXXXXXXXXX" );
	fd = mkstemp( nameTemplate );
#else
	snprintf( nameTemplate, sizeof( nameTemplate ), "breve_temp.%lu",
	          ( unsigned long )random() % 10000000 );
