...
class Swarm( breve.Control ):
	...
	def init( self ):
		...
		self.agentImage = breve.createInstances( breve.Image, 1 ).load( 'images/arrow.png' )
		...
		self.birds.setBitmapImage( self.agentImage )
		self.birds.setColor( breve.randomExpression( breve.vector( 1, 1, 1 ) ) )
		self.flockNormally()
...
class Bird( breve.Mobile ):
	...
	def fly( self ):
		...
		self.setBitmapHeadingPoint( self.getVelocity() )

	def getAngle( self, otherMobile ):
		tempVector = breve.vector()

		tempVector = ( otherMobile.getLocation() - self.getLocation() )
		if ( breve.length( tempVector ) == 0.000000 ):
			return 0

		if ( breve.length( self.getVelocity() ) == 0.000000 ):
			return 0
