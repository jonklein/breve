...
class Swarm( breve.Control ):
	def __init__( self ):
		...
		self.selection = None
		self.vision = None
		self.visionDirection = breve.vector()
		self.wackyMenu = None
		Swarm.init( self )
	...
	def init( self ):
		...
		self.vision = breve.createInstances( breve.Camera, 1 )
		self.vision.setSize( 200, 200 )
		self.vision.setPosition( 0, 0 )
...
class Bird( breve.Mobile ):
	...
	def getAngle( self, otherMobile ):
		...
		if ( ( breve.length( tempVector ) == 0.000000 ) or ( breve.length( velocity ) == 0.000000 ) ):
			return 0.000000
