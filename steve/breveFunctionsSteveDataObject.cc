...
char *stPackObject( stInstance *i, int *length ) {
	...
	*length = sizeof( brDataObjectHeader ) + ( i->type->varSize - i->type->varOffset ) + i->type->name.size();
	...
	header->nameLength = i->type->name.size();

	header->magic = SDO_MAGIC_NUMBER;

	header->version = SDO_VERSION;

	strcpy( &package[sizeof( brDataObjectHeader )], i->type->name.c_str() );
