...
void slVclipDataInit( slWorld *w ) {
	...
	// for each object in the world, fill in its shape, position and min/max vectors
	...
	for ( x = 0; x < w->_clipData->_maxCount; x++ )
		memset( w->_clipData->pairArray[ x ], ( BT_CHECK | BT_UNKNOWN ), w->_clipData->_maxCount );
...
slVclipData::slVclipData() {
	...
	pairArray = new unsigned char*[ _maxCount ];

	for ( unsigned int n = 0; n < _maxCount; ++n )
		pairArray[ n ] = new unsigned char[ _maxCount ];
}

slVclipData::~slVclipData() {
	if ( !_maxCount ) return;

	for ( unsigned int n = 0; n < _maxCount; ++n ) delete[] pairArray[n];
...
void slVclipData::realloc( int c ) {
	...
	while ( _count >= _maxCount ) _maxCount *= 2;
...
void slInitProximityData( slWorld *w ) {
	...
	for ( n = 0; n < w->_proximityData->_maxCount; n++ )
		memset( w->_proximityData->pairArray[n], BT_CHECK, w->_proximityData->_maxCount );
